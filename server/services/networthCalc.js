...
async function calculateAndStoreNetworth(provinceId) {
  const [provinceRes, buildingsRes, troopsRes, researchRes] = await Promise.all([
    pool.query('SELECT land, gold FROM provinces WHERE id = $1', [provinceId]),
    pool.query('SELECT SUM(level) as total_levels FROM province_buildings WHERE province_id = $1', [provinceId]),
    pool.query(
    ...
    pool.query(
      `SELECT COUNT(*) as completed FROM province_research WHERE province_id = $1 AND status = 'complete'`,
      [provinceId]
    ),
  ]);
  ...
  const buildingValue = parseInt(buildingsRes.rows[0].total_levels || 0) * 500;
  const troopValue = Math.floor(parseFloat(troopsRes.rows[0].troop_value || 0) * 0.5);
  const goldValue = Math.floor(province.gold * 0.1);
  const techValue = parseInt(researchRes.rows[0].completed) * 1000;
